...
const report = JSON.parse(output) as {
  ...
  appVersion: string;
  generatedAt: string;
  summary: { explainers: number; pending: number; staleSources: number; warnings: number; actions: number; freshness: { current: number; reviewNeeded: number; invalidDates: number }; coverage: { explainers: number; reviewed: number; pending: number; sources: number; actions: number; impactCount: number; highRiskImpactCount: number; impactsWithScenarios: number } };
  rows: Array<{ priority: number; slug: string; title: string; sourceCount: number; impactCount: number; highRiskImpactCount: number; impactsWithScenarios: number; sources: Array<{ id: string; title: string; url: string; accessedAt: string; validity: string; freshness: { ageDays: number | null; dueAt: string | null; status: string; reason: string } }>; actions: Array<{ id: string; kind: string; priority: string; sourceIds: string[] }> }>;
  priorityRule: { pending: number; staleSource: number; warning: number };
};

assert.match(report.generatedAt, /^\d{4}-\d{2}-\d{2}T/);
assert.equal(report.schemaVersion, "1.4");
assert.equal(report.appVersion, "0.223.0");
...
assert.equal(report.summary.freshness.invalidDates, 0);
...
assert.ok(report.rows.every((row) => row.sources.every((source) => ["within-window", "manual-review", "outside-window", "invalid-date"].includes(source.freshness.reason))));

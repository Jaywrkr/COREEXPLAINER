...
import { canAdvanceWorkflow, isWorkflowStatus, nextWorkflowStatus, type WorkflowStatus, workflowStageFor, workflowStageIndex, workflowStages } from "@/lib/review/workflowState";
...
export function ContentWorkflowPanel({ slug, meta }: ContentWorkflowPanelProps) {
  ...
  const currentStage = workflowStageFor(status);
  const guards = { technicalReviewComplete: meta.reviewStatus === "reviewed", sourcesCurrent: staleSources === 0 };
  const canAdvance = canAdvanceWorkflow(status, guards);

  useEffect(() => {
    const saved = window.localStorage.getItem(storageKey);
    if (isWorkflowStatus(saved)) setStatus(saved);
    setReady(true);
  }, [storageKey]);
  ...
  const advance = () => {
    ...
    const next = nextWorkflowStatus(status, guards);
    if (next) { setStatus(next); recordProductEvent({ name: "workflow-advance", slug, id: next }); }
  };
  const markDue = () => { setStatus("review-due"); recordProductEvent({ name: "workflow-advance", slug, id: "review-due" }); };
    ...
    <details className="border-t border-core-border/[0.1] pt-2">
      ...
      <div className="mt-2 space-y-2 text-[0.62rem] leading-relaxed">
        <div className="flex flex-wrap gap-1">
          {workflowStages.map((stage, index) => <span key={stage.id} className={`border px-1.5 py-0.5 ${stage.id === status ? "border-core-warning/60 text-core-warning" : index < stageIndex && status !== "review-due" ? "border-core-success/40 text-core-success" : "border-core-border/[0.14] text-core-text-muted"}`}>{stage.label}</span>)}
        </div>
        <p className="text-core-text-secondary"><span className="font-semibold text-core-text">Responsable:</span> {currentStage.role}</p>
        {status === "review-due" ? <p className="text-core-warning">La revisión venció; el siguiente paso vuelve a revisión técnica antes de permitir otra publicación.</p> : null}
        ...
        <div className="flex flex-wrap gap-1.5">
          <button type="button" onClick={advance} disabled={!canAdvance} className="border border-core-accent/40 px-2 py-1 font-semibold text-core-accent hover:bg-core-accent/10 disabled:cursor-not-allowed disabled:opacity-50">Avanzar etapa</button>

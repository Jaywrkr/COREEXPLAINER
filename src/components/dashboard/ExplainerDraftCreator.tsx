...
interface CreatorResponse { draft?: ExplainerDraft; fallback?: boolean; generatedBy?: "ai" | "local-template"; message?: string; usage?: { totalTokens?: number; estimatedCostUsd?: number }; policy?: CreatorPolicy }
...
export function ExplainerDraftCreator() {
  ...
  const [draftOrigin, setDraftOrigin] = useState<"ai" | "local-template" | null>(null);
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (payload.draft) { setDraft(payload.draft); setDraftOrigin(payload.generatedBy === "ai" ? "ai" : "local-template"); setStatus("Borrador estructurado recibido. Validar técnicamente antes de publicarlo."); }
      else { setDraft(localDraft(topic, audience, brands, goal)); setDraftOrigin("local-template"); setStatus(`${payload.message ?? "IA no disponible"} Se generó una plantilla local editable.`); }
    } catch { setUsage(recordAiUsage(false, 0, 0, "creator")); setDraft(localDraft(topic, audience, brands, goal)); setDraftOrigin("local-template"); setStatus("No se pudo conectar con IA. Se generó una plantilla local editable."); }
  ...
  return (
    <section className="mb-14 border border-core-accent/25 bg-core-panel/40 p-5 sm:p-6">
      ...
      <p className="mt-3 border-l-2 border-core-accent/50 pl-2 text-[0.62rem] leading-relaxed text-core-text-muted">Política: borrador educativo solamente; no se publica ni ejecuta. {policy ? `Presupuesto estimado: ${policy.estimatedInputTokens} tokens de entrada · máximo ${policy.maxOutputTokens} de salida${policy.estimatedCostUsd !== undefined ? ` · hasta US$ ${policy.estimatedCostUsd.toFixed(4)}` : " · coste no configurado"}.` : "El coste solo se estima si el entorno declara tarifas."}</p>
      ...
      {draft ? <div className="mt-4 border-t border-core-border/[0.12] pt-4"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-core-text">{draft.title}</h3><span className="border border-core-warning/40 px-1.5 py-0.5 font-mono text-[0.52rem] uppercase text-core-warning">{draftOrigin === "ai" ? "origen: IA · revisar" : "origen: plantilla local"}</span></div><p className="mt-1 text-sm text-core-text-secondary">{draft.tagline}</p><div className="mt-3 grid gap-2 sm:grid-cols-3">{draft.scenes.map((scene) => <article key={scene.id} className="border border-core-border/[0.12] p-3"><p className="text-xs font-semibold text-core-text">{scene.title}</p><p className="mt-1 text-xs leading-relaxed text-core-text-secondary">{scene.businessImpact}</p></article>)}</div><p className="mt-3 text-xs text-core-text-muted">Pendientes de validar: {draft.validationGaps.join(" · ")}</p></div> : null}
    </section>
  );
}

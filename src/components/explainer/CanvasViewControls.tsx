...
export function CanvasViewControls({ scale, technical, motionPaused, onToggleMotion, onZoomIn, onZoomOut, onReset, onFit }: CanvasViewControlsProps) {
  ...
  if (technical) {
    return (
      <details open={open} onToggle={(event) => setOpen(event.currentTarget.open)} className="absolute left-4 top-4 z-20 border border-core-border/[0.14] bg-core-panel/95 font-mono text-[0.6rem] text-core-text shadow-sm backdrop-blur-sm">
        <summary className="cursor-pointer list-none px-2.5 py-2 text-core-text-muted transition-colors hover:text-core-text [&::-webkit-details-marker]:hidden">Vista <span className="text-core-text">{percent}</span> <span aria-hidden="true" className="ml-1 text-core-accent">⌄</span></summary>
        <div className="flex flex-wrap border-t border-core-border/[0.14]">
  ...
  return (
    <details open={open} onToggle={(event) => setOpen(event.currentTarget.open)} className="absolute left-4 top-4 z-20 border border-core-border/[0.14] bg-core-panel/90 font-mono text-[0.6rem] text-core-text shadow-sm backdrop-blur-sm">
      ...
      <div className="flex flex-wrap border-t border-core-border/[0.12]">
        <button type="button" onClick={onZoomOut} className="px-3 py-2 transition-colors hover:bg-core-accent/10 hover:text-core-text" aria-label="Alejar diagrama">−</button>

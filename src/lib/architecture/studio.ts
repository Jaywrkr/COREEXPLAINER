...
export const studioCatalog: StudioCatalogComponent[] = [
  ...
  { id: "vmware-host", vendor: "VMware by Broadcom", name: "Host VMware", kind: "compute", description: "Host de virtualización; modelo, NIC, HBA y release se validan en ingeniería.", ports: [{ id: "eth", label: "Ethernet / VMkernel", type: "ethernet" }, { id: "fc", label: "HBA Fibre Channel", type: "fibre-channel" }, { id: "iscsi", label: "iSCSI", type: "iscsi" }, { id: "mgmt", label: "Gestión", type: "management" }] },
  { id: "lenovo-server", vendor: "Lenovo", name: "ThinkSystem", kind: "compute", description: "Servidor físico para cómputo o plataforma.", ports: [{ id: "eth", label: "Ethernet", type: "ethernet" }, { id: "fc", label: "HBA Fibre Channel", type: "fibre-channel" }, { id: "iscsi", label: "iSCSI", type: "iscsi" }, { id: "mgmt", label: "Gestión", type: "management" }] },
...
export function normalizeGeneratedDiagram(input: unknown): StudioDiagram | null {
  ...
  const nodes = candidate.nodes.slice(0, 16).flatMap((node, index) => {
    ...
    return [{ id: typeof value.id === "string" && value.id ? value.id.slice(0, 48) : `node-${index + 1}`, componentId: value.componentId, label: typeof value.label === "string" && value.label.trim() ? value.label.trim().slice(0, 72) : componentById(value.componentId)!.name, x: typeof value.x === "number" ? value.x : 12 + ((index % 4) * 25), y: typeof value.y === "number" ? value.y : 18 + (Math.floor(index / 4) * 30) }];
  });
